...
class MSAEmbeddings(nn.Module):
    ...
    def forward(self, seq_ids, msa_ids, position_ids):
        ...
        return: (B, K, L, C)
        """
        B, K, _, L = msa_ids.shape    ##### _ is 17
        seq = self.onehot(seq_ids)    ##### (B, L) --> (B, L, 24): [1, 76, 24]
        msa_ids = msa_ids.transpose(-2, -1)    ##### [1, 61, 76, 17]
        boundary = msa_ids[..., -2:].float()    ##### [1, 61, 76, 2]
        msa = self.onehot(msa_ids[..., :-2]).reshape(B, K, L, -1)    ##### [1, 61, 76, 15]
        msa = torch.cat([seq[:, None].repeat(1, msa.shape[1], 1, 1), msa, boundary], dim=-1)    ##### [1, 61, 76, 386]
        msa_emb = self.msa_embeddings(msa)    ##### [1, 61, 76, 128]
